...
class Items extends Secure_area implements iData_controller
{
	...
	*/
	...
	function delete()
	{
        ...
        function activar()
        {
                $items_to_activar=$this->input->post('ids');

                if($this->Item->activar_list($items_to_activar))
                {
                        echo json_encode(array('success'=>true,'message'=>$this->lang->line('items_successful_deleted').' '.
                        count($items_to_activar).' '.$this->lang->line('items_one_or_multiple')));
                }
                else
                {
                        echo json_encode(array('success'=>false,'message'=>$this->lang->line('items_cannot_be_deleted')));
