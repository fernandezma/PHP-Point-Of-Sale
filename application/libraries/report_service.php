...
class Report_Service
{
    ...
    function inventory_low($export_excel=0)
	{
		...
		foreach($report_data as $row)
		{
			$tabular_data[] = array($row['item_number'], $row['name'], $row['category'], $row['description'],$row['quantity'], $row['reorder_level'], $row['cost_price']);
		}
    ...
    function inventory_summary($export_excel=0)
	{
		...
		foreach($report_data as $row)
		{
			$tabular_data[] = array($row['item_number'],$row['location'], $row['name'], $row['description'], $row['quantity'], $row['reorder_level'], );
		}
